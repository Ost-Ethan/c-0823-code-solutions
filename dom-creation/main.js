...
function renderPokemon(pokemon) {
  ...
  const $pokeImageImg = document.createElement('img');
  const $cardTextDiv = document.createElement('div');
  const $pokeNameH2 = document.createElement('h2');
  const $pokeNumberH3 = document.createElement('h3');
  const $pokeDescriptionP = document.createElement('p');

  $columnDiv.className = 'column-third';
  $pokeCardDiv.className = 'pokemon-card';
  $pokeImageImg.setAttribute('src', pokemon.imageUrl);
  ...
  $cardTextDiv.appendChild($pokeNameH2);
  $cardTextDiv.appendChild($pokeNumberH3);
  $cardTextDiv.appendChild($pokeDescriptionP);
...
for (let i = 0; i < pokedex.length; i++) {
  const pokemonRender = renderPokemon(pokedex[i]);

  $row.appendChild(pokemonRender);
}
