...
export function Todos() {
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      ...
      setTodos((prev) => prev.concat(newEntry));
    } catch (err) {
      console.log(err);
      setError(err);
    } finally {
  ...
  async function toggleCompleted(todoId: number) {
    const clickedToDo = todos.find((item) => {
      return item.todoId === todoId;
    });
    const completedObject = { isCompleted: !clickedToDo?.isCompleted };
    ...
    try {
      ...
      const updatedArray = todos.map((item) => {
        if (item.todoId === updatedEntry.todoId) {
          return updatedEntry;
        } else {
          return item;
        }
